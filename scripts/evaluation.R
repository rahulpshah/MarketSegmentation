...
# ===== Reading the graph ====== #
g <- read.graph(file="~/Documents/06.Topic-7.Project-6.MarketSegmentation.AttributedGraphCommunityDetection/data/fb_caltech_small_edgelist.txt", format = c("edgelist"))

# ===== Read communities file ==== #
comm <- integer(vcount(g))

conn <- file("~/Documents/06.Topic-7.Project-6.MarketSegmentation.AttributedGraphCommunityDetection/communities_1.txt", open="r")
...
attrData <- read.csv("~/Documents/06.Topic-7.Project-6.MarketSegmentation.AttributedGraphCommunityDetection/data/fb_caltech_small_attrlist.csv")
akm = akmeans(attrData, d.metric=2, ths3=0, mode=3, 
              min.k=length(unique(comm)), max.k=length(unique(comm))+5) ## cosine distance based
